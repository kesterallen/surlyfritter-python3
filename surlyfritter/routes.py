...
def image_perm(added_order: int):
    ...
    with client.context():
        try:
            ...
            if picture is None:
                print(
                    f"""no Picture with .added_order of {added_order},
            ...
            abort(
                404,
                f"blob for added_order={added_order} not found in image_perm; {err}",
            )
...
def gone_with_the_wind():
    ...
    def _img_from_url(url, newsize=None):
        ...
        resp.raw.decode_content = True  # Content-Encoding
